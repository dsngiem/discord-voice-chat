...
const insertCSS = (opts, attempt = 0) => {
  console.log("Injecting CSS into webview")
...
const injectContentScripts = async details => {
  ...
  // if (true) {
    ...
    CONTENT_CSS.forEach(script => {
      if (!script.matches.some(matchesPattern.bind(null, url))) return
      insertCSS({ tabId, frameId, file: script.file })
    })
  }
}
